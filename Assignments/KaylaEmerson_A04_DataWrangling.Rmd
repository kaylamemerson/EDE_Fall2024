...
#2 
#reveal dimensions
dim(EPAair.O3.NC2018.data)
dim(EPAair.O3.NC2019.data)
dim(EPAair.PM25.NC2018.data)
dim(EPAair.PM25.NC2019.data)
```
...
O3.PM25.combined <- 
  ...
  group_by(Date, AQS_PARAMETER_DESC, COUNTY) %>%
  filter(!is.na(DAILY_AQI_VALUE) & !is.na(SITE_LATITUDE) & !is.na(SITE_LONGITUDE)) %>%
  summarize(meanAQI = mean(DAILY_AQI_VALUE)) %>%
  summarize(meanLat = mean(SITE_LATITUDE)) %>%
  summarize(meanLong = mean(SITE_LONGITUDE)) 
