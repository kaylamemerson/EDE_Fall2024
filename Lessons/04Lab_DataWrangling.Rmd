...
# %in% means multiple values, is __ in the list?
...
#slice is c() meaning columns _ through _ as c(_:_)
```
...
NTL.phys.data.Tuesday <-
  filter(NTL.phys.data, year4 %in% c(1990:1999) & lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: row numbers may change; filter on data itself not how its organized 
