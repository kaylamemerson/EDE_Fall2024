...
huc8_sf_2 <- st_read(here('Data/Raw/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == 'Upper Neuse') #filter for just upper neuse subbasin

#Create a map
mapview(huc8_sf, col.regions = "gray") +
  mapview(huc8_sf_2, col.regions = "yellow", color = "darkblue")


ggplot(data=huc8_sf) +
  geom_sf() +
  geom_sf(data = huc8_sf_2, fill = "yellow", color = "blue")
...
#View the outputs
mapview(wind_sf_join, zcol = "wind_mean")
```
...
triCo_in30km <- st_buffer(triCo, -30000)
myMap <- myMap + geom_sf(data = triCo_in30km, color = 'orange', fill=NA)
myMap                            
```
