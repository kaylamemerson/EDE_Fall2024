...
Rainfall_BR_processed <-
  Rainfall_BR %>% 
  rename( Rainfall = Rainfall....MM.) %>% #rename col
  separate(Statistics,c("Null","Month","Null2")," ") %>% #extract month information 
  mutate( Date = my(paste0(Month,"-",Year))) %>% #create a date object 
  select(Date,Rainfall) #select columns of interest 
...
#No NAs
#but don't use this method to search for NAs...
...
#bars on right side show how much the trend/seasonal/remainder has to "zoom in" 
